...
test_that("quantileResiduals gives correct residuals for non-restricted models", {
  ...
  expect_equal(quantileResiduals_int(VIX, 1, 2, params12t, model="StMAR")[13:17], c(-0.97288160, 1.48243917, 0.39651300, -0.18067370, -0.01133098), tolerance=1e-3)
  expect_equal(quantileResiduals_int(VIX[1:10], 2, 2, params22)[1:2], c(0.5079722, -0.3657400), tolerance=1e-3)
  expect_equal(quantileResiduals_int(3*VIX, 2, 2, params22)[113:115], c(5.578132, 4.201229, 3.801425), tolerance=1e-3)
  ...
  expect_equal(quantileResiduals_int(3*VIX, 2, 3, params23)[1:3], c(5.053051, 2.367575, 5.296035), tolerance=1e-3)
  expect_equal(quantileResiduals_int(VIX, 1, c(1, 1), params12gs, model="G-StMAR")[10:12], c(0.7802472, 0.8500172, 1.1435514), tolerance=1e-3)
...
test_that("quantileResiduals gives correct residuals for restricted models", {
  expect_equal(quantileResiduals_int(VIX, 1, 2, params12r, restricted=TRUE)[1:3], c(-0.7925692, -0.1961077, -2.2397076), tolerance=1e-3)
  expect_equal(quantileResiduals_int(VIX, 1, 2, params12r, restricted=TRUE)[12:14], c(1.163688, -1.467829, 1.058580), tolerance=1e-3)
  ...
  expect_equal(quantileResiduals_int(VIX, 2, 3, params23tr, model="StMAR", restricted=TRUE)[16:18], c(0.33635368, -0.06784057, 0.62990153), tolerance=1e-3)
  ...
  expect_equal(quantileResiduals_int(VIX, 1, c(2, 2), params14gsr, model="G-StMAR", restricted=TRUE)[70:72], c(0.6686324, 0.8246961, 0.2662351), tolerance=1e-3)
})

test_that("quantileResiduals gives correct residuals for constrained models", {
  expect_equal(quantileResiduals_int(VIX, 2, 1, params21c, model="StMAR", constraints=list(R3))[240:241], c(-0.3792682, 1.0279554), tolerance=1e-3)
  ...
  expect_equal(quantileResiduals_int(3*VIX, 2, 2, params22c_2, constraints=list(R4, R4))[113:115], c(5.578132, 4.201229, 3.801425), tolerance=1e-3)
  expect_equal(quantileResiduals_int(VIX, 2, 2, params22cr, model="StMAR", restricted=TRUE, constraints=R3)[24:25], c(0.4021366, 0.6204558), tolerance=1e-3)
  ...
  expect_equal(quantileResiduals_int(VIX, 2, 3, params23cr, restricted=TRUE, constraints=R4)[130:131], c(0.4080434, 0.3146683), tolerance=1e-3)
  expect_equal(quantileResiduals_int(VIX, 3, c(1, 1), params32gsc, model="G-StMAR", restricted=FALSE, constraints=list(R1, R2))[13:14], c(1.219363, 1.113442), tolerance=1e-3)
