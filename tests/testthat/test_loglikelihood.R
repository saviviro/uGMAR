...
test_that("mixing_weights gives correct values for non-restricted models", {
  expect_equal(mixing_weights_int(simudata, 1, c(1, 1), params12gs, model="G-StMAR")[10, 2], 0.9631588, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 2, c(1, 2), params23gs, model="G-StMAR")[20, 3], 0.2213917, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 1, 2, params12, model="GMAR")[2, 1], 0.656533, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 2, 2, params22t, model="StMAR")[1, 1], 0.007149177, tolerance=1e-4)
  expect_equal(mixing_weights_int(simudata, 2, 3, params23, model="GMAR")[13, 1], 0.8337944, tolerance=1e-3)
})

test_that("mixing_weights gives correct values for restricted models", {
  expect_equal(mixing_weights_int(simudata, 1, c(2, 1), params13gsr2, model="G-StMAR", restricted=TRUE)[20, 2], 0.9781194, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 1, c(2, 1), params13gsr, model="G-StMAR", restricted=TRUE)[40, 1], 0.01115213, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 2, 3, params23r, model="GMAR", restricted=TRUE)[13, 2], 0.1214211, tolerance=1e-6)
  expect_equal(mixing_weights_int(simudata, 2, 3, params23tr, model="StMAR", restricted=TRUE)[5, 3], 0.9339346, tolerance=1e-6)
})

test_that("mixing_weights gives correct values for constrained models", {
  expect_equal(mixing_weights_int(simudata, 3, c(1, 1), params32gsc, model="G-StMAR", constraints=list(R1, R2))[1, 2], 1, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 2, c(1, 1), params22gsrc, model="G-StMAR", restricted=TRUE, constraints=R3)[1, 2], 1, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 3, 2, params32c, model="StMAR", constraints=list(R1, R1))[1, 1], 0.008782872, tolerance=1e-6)
  expect_equal(mixing_weights_int(simudata, 3, 3, params33c, model="GMAR", constraints=list(R2, R2, R1))[113, 3], 0.0001672733, tolerance=1e-6)
  expect_equal(mixing_weights_int(simudata, 2, 1, params21c, model="StMAR", constraints=list(R3))[100, 1], 1, tolerance=1e-3)
  expect_equal(mixing_weights_int(simudata, 3, 2, params32cr, model="GMAR", restricted=TRUE, constraints=R1)[150, 2], 1, tolerance=1e-3)
})

test_that("cond_moments gives correct values for non-restricted models", {
  expect_equal(cond_moments(simudata, 1, c(1, 1), params12gs2, model="G-StMAR", to_return="regime_cmeans")[10,], c(10.47718, 11.87718), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, c(1, 1), params12gs2, model="G-StMAR", to_return="regime_cvars")[10,], c(1.500000, 2.484769), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, c(1, 1), params12gs, model="G-StMAR", to_return="total_cmeans")[1:2], c(12.70827, 13.03498), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, c(1, 1), params12gs, model="G-StMAR", to_return="total_cvars")[111:112], c(0.8466878, 0.5714409), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 2, c(1, 2), params23gs, model="G-StMAR", to_return="regime_cmeans")[20,], c(3.396052, 5.992103, 1.216248), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 2, c(1, 2), params23gs, model="G-StMAR", to_return="regime_cvars")[20,], c(1.00000, 14.02534, 14.39489), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 2, c(1, 2), params23gs, model="G-StMAR", to_return="total_cvars")[20], 10.17544, tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, 2, params12, model="GMAR", to_return="regime_cmeans")[2,], c(12.70238, 13.56910), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, 2, params12, model="GMAR", to_return="regime_cvars")[21,], c(0.30, 1.98), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 2, 3, params23, model="GMAR", to_return="total_cmeans")[13], 10.74125, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 2, 3, params23, model="GMAR", to_return="total_cvars")[100], 2.426266, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 1, 2, params12t, model="StMAR", to_return="total_cmeans")[1], 12.6818, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 1, 2, params12t, model="StMAR", to_return="total_cvars")[2], 1.039859, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 2, 2, params22t, model="StMAR", to_return="regime_cmeans")[1,], c(12.38894, 12.86807), tolerance=1e-4)
  expect_equal(cond_moments(simudata, 2, 2, params22t, model="StMAR", to_return="regime_cvars")[2,], c(0.2964307, 1.3786931), tolerance=1e-4)
})

test_that("cond_moments gives correct values for restricted models", {
  expect_equal(cond_moments(simudata, 1, c(2, 1), params13gsr2, model="G-StMAR", restricted=TRUE, to_return="regime_cmeans")[30,], c(10.61347, 10.31347, 10.71347), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, c(2, 1), params13gsr2, model="G-StMAR", restricted=TRUE, to_return="regime_cvars")[30,], c(0.4000, 2.000, 0.9556815), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 1, c(2, 1), params13gsr, model="G-StMAR", restricted=TRUE, to_return="total_cmeans")[40], 10.33982, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 1, c(2, 1), params13gsr, model="G-StMAR", restricted=TRUE, to_return="total_cvars")[40], 0.2094651, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 1, 2, params12r, model="GMAR", restricted=TRUE, to_return="total_cmeans")[100:101], c(12.79224, 13.15661), tolerance=1e-4)
  expect_equal(cond_moments(simudata, 1, 2, params12tr, model="StMAR", restricted=TRUE, to_return="total_cvars")[11:12], c(3.306345, 4.085636), tolerance=1e-6)
  expect_equal(cond_moments(simudata, 2, 3, params23r, model="GMAR", restricted=TRUE, to_return="regime_cmeans")[13,], c(9.744875, 9.944875, 10.144875), tolerance=1e-6)
  expect_equal(cond_moments(simudata, 2, 3, params23tr, model="StMAR", restricted=TRUE, to_return="regime_cvars")[111,], c(0.8589741, 1.1162082, 3.5958124), tolerance=1e-6)
})

test_that("cond_moments gives correct values for constrained models", {
  expect_equal(cond_moments(simudata, 3, c(1, 1), params32gsc, model="G-StMAR", constraints=list(R1, R2), to_return="total_cmeans")[1], 9.890363, tolerance=1e-2)
  expect_equal(cond_moments(simudata, 2, c(1, 1), params22gsrc, model="G-StMAR", restricted=TRUE, constraints=R3, to_return="total_cvars")[1], 11.30548, tolerance=1e-4)
  expect_equal(cond_moments(simudata, 3, 2, params32c, model="StMAR", constraints=list(R1, R1), to_return="regime_cmeans")[1,], c(1.115771, 2.231541), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 3, 3, params33c, model="GMAR", constraints=list(R2, R2, R1), to_return="total_cvars")[113], 1.994041, tolerance=1e-6)
  expect_equal(cond_moments(simudata, 2, 1, params21c, model="StMAR", constraints=list(R3), to_return="regime_cvars")[100,], 1.228252, tolerance=1e-6)
  expect_equal(cond_moments(simudata, 2, 2, params22c, model="StMAR", constraints=list(R4, R3), to_return="regime_cmeans")[2, ], c(1.088879, 4.677675), tolerance=1e-3)
  expect_equal(cond_moments(simudata, 2, 2, params22cr, model="StMAR", restricted=TRUE, constraints=R3, to_return="total_cmeans")[10:11], c(11.21663, 11.71482), tolerance=1e-6)
  expect_equal(cond_moments(simudata, 3, 2, params32cr, model="GMAR", restricted=TRUE, constraints=R1, to_return="total_cvars")[100], 2, tolerance=1e-6)
})
