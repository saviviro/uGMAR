...
test_that("parameterChecks throws errors correctly", {
  ...
  expect_error(parameterChecks(1, 1, params11, pars=as.matrix(params11[1:3]), alphas=1, StMAR=FALSE))
  ...
  expect_error(parameterChecks(1, 2, params12, pars=matrix(params12[1:6], ncol=2), alphas=c(0.7, 0.3), StMAR=TRUE))
...
test_that("checkConstraintMat throws errors correctly", {
  expect_error(checkConstraintMat(2, 2, R=list(R1, R2)))
  expect_error(checkConstraintMat(3, 2, R=list(R1, R1, R1)))
  expect_error(checkConstraintMat(2, 2, R=R2))
  expect_error(checkConstraintMat(2, 2, R=list(R3, R3)))
  expect_error(checkConstraintMat(3, 1, R=list(R2)))
  expect_error(checkConstraintMat(3, 1, R=list(as.vector(R3))))

  expect_error(checkConstraintMat(2, 3, R=R1, restricted=TRUE))
  expect_error(checkConstraintMat(3, 1, R=R2, restricted=TRUE))
  expect_error(checkConstraintMat(2, 2, R=R3, restricted=TRUE))
  expect_error(checkConstraintMat(3, 3, R=R3, restricted=TRUE))
  expect_error(checkConstraintMat(3, 3, R=as.vector(R4), restricted=TRUE))
})
...
test_that("nParams works correctly", {
  ...
  expect_equal(nParams(1, 2, StMAR=TRUE, restricted=TRUE), length(params12tr))
  expect_equal(nParams(2, 3, restricted=TRUE), length(params23r))
  expect_equal(nParams(3, 2, StMAR=TRUE, constraints=TRUE, R=list(R1, R1)), length(params32c))
  expect_equal(nParams(3, 3, constraints=TRUE, R=list(R2, R2, R1)), length(params33c))
