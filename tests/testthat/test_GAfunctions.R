...
test_that("change_regime changes the right regime correctly", {
  expect_equal(change_regime(2, c(1, 2), params23gsr, model="G-StMAR", restricted=TRUE, regimeParams=c(7, 7, 70), regime=3), c(1, 2, 7, 0.5, 0.05, 1, 2, 7, 0.4, 0.4, 20, 70))
  expect_equal(change_regime(2, c(1, 2), params23gsr, model="G-StMAR", restricted=TRUE, regimeParams=c(7, 7), regime=1), c(7, 2, 3, 0.5, 0.05, 7, 2, 3, 0.4, 0.4, 20, 30))
  expect_equal(change_regime(2, c(1, 1), params22gs, model="G-StMAR", regimeParams=c(3, 0.3, 0.3, 3, 30), regime=2), c(1, 0.1, 0.1, 1, 3, 0.3, 0.3, 3, 0.3, 30))
  expect_equal(change_regime(2, c(1, 1), params22gs, model="G-StMAR", regimeParams=c(3, 0.3, 0.3, 3), regime=1), c(3, 0.3, 0.3, 3, 2, 0.2, 0.2, 2, 0.3, 10))

  expect_equal(change_regime(1, 2, params12, regimeParams=c(99, 98, 97), regime=1), c(99.0, 98.0, 97.0, 4.5, 0.7, 3.0, 0.8))
  expect_equal(change_regime(2, 2, params22t, model="StMAR", regimeParams=c(9, 8, 7, 6, 5), regime=2), c(1.4, 0.8, 0.05, 0.27, 9, 8, 7, 6, 0.7, 203, 5))
  expect_equal(change_regime(2, 3, params23, regimeParams=c(9, 8, 7, 6), regime=3), c(2.7, 0.8, -0.06, 0.3, 3.5, 0.8, -0.07, 2.6, 9, 8, 7, 6, 0.6, 0.25))
  expect_equal(change_regime(1, 2, params12tr, model="StMAR", restricted=TRUE, regimeParams=c(9, 8, 7), regime=2), c(0.8, 9, 0.9, 0.4, 8, 0.9, 4, 7))
  expect_equal(change_regime(2, 3, params23r,  restricted=TRUE, regimeParams=c(11, 12), regime=2), c(1.7, 11, 2.1, 0.8, -0.05, 0.3, 12, 4.5, 0.7, 0.2))
  expect_equal(change_regime(2, 3, params23tr, model="StMAR", restricted=TRUE, regimeParams=c(99, 88, 77), regime=3), c(1.9, 1.6, 99, 0.8, -0.02, 0.4, 0.1, 88, 0.6, 0.3, 15, 200, 77))

  expect_equal(change_regime(2, 1, params21c, model="StMAR", constraints=list(R3), regimeParams=c(9, 8, 7, 6), regime=1), c(9, 8, 7, 6))
  expect_equal(change_regime(2, 2, params22c, model="StMAR", constraints=list(R4, R3), regimeParams=c(9, 8, 7, 6), regime=2), c(1, 0.1, -0.1, 1, 9, 8, 7, 0.8, 11, 6))
  expect_equal(change_regime(3, 3, params33c, constraints=list(R2, R2, R1), regimeParams=c(9, 8, 7, 6), regime=3), c(1, 0.1, 0.1, 0.1, 1, 2, 0.2, 0.2, 0.2, 2, 9, 8, 7, 6, 0.5, 0.4))
  expect_equal(change_regime(2, 1, params21cr, restricted=TRUE, constraints=R3, regimeParams=c(9, 8), regime=1), c(9, 1, 8))
  expect_equal(change_regime(2, 2, params22cr, model="StMAR", restricted=TRUE, constraints=R3, regimeParams=c(9, 8, 7), regime=1), c(9, 2, 0.8, 8, 2, 0.7, 7, 12))
...
test_that("GA functions don't throw errors", {
  ...
  test_length0(random_ind_int(p=1, M=1, model="StMAR", meanscale=1:2, sigmascale=1), 4)
  test_length0(smart_ind_int(p=1, M=2, params=params12, model="GMAR", meanscale=1:2, sigmascale=1, accuracy=1, whichRandom=2), 7)
})
