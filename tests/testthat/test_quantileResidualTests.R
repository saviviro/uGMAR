...
test_that("quantile residual test for normality works", {
  ...
  expect_equal(qrt23t$norm_res$testStat, 8.00678, tolerance=1e-4)
...
test_that("quantile residuals tests for autocorrelation work", {
  ...
  expect_equal(qrt23t$ac_res$testStat, 0.6429272, tolerance=1e-4)
  expect_equal(qrt23t$ac_res$indStat, -0.02966591, tolerance=1e-4)
  expect_equal(qrt23t$ac_res$stdError, 0.09147289, tolerance=1e-4)
  ...
  expect_equal(qrt12r$ac_res$testStat[2], 1.459883, tolerance=1e-4)
  expect_equal(qrt12$ac_res$stdError[1], 0.06842647, tolerance=1e-4)
})
...
test_that("quantile residual tests for conditional heteroskedasticity work", {
  ...
  expect_equal(qrt12$ch_res$indStat[3], -0.4115146, tolerance=1e-4)
  expect_equal(qrt12$ch_res$stdError[4], 0.122487, tolerance=1e-4)
  ...
  expect_equal(qrt23t$ch_res$testStat, 6.542458, tolerance=1e-4)
  expect_equal(qrt23t$ch_res$indStat, -0.3604339, tolerance=1e-4)
  expect_equal(qrt23t$ch_res$stdError, 0.1397589, tolerance=1e-4)
  expect_equal(qrt12r$ch_res$testStat[1], 0.07133745, tolerance=1e-4)
  expect_equal(qrt12r$ch_res$testStat[2], 0.43759, tolerance=1e-4)
  expect_equal(qrt12r$ch_res$indStat, c(0.1396549, -0.2607705), tolerance=1e-4)
  expect_equal(qrt12r$ch_res$stdError[2], 0.4147108, tolerance=1e-4)
})
