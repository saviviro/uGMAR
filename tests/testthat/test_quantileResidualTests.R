...
# in precision of the calculations might accumulate to differences big enough for the tests to fail. As the
...
test_that("quantile residual test for normality works", {
  ...
  expect_equal(qrt23t$norm_res$testStat, 8.00678, tolerance=1e-3)
...
test_that("quantile residuals tests for autocorrelation work", {
  ...
  expect_equal(qrt23t$ac_res$testStat, 0.6429272, tolerance=1e-3)
  expect_equal(qrt23t$ac_res$indStat, -0.02966591, tolerance=1e-3)
  expect_equal(qrt23t$ac_res$stdError, 0.09147289, tolerance=1e-3)
...
test_that("quantile residual tests for conditional heteroskedasticity work", {
  ...
  expect_equal(qrt23t$ch_res$testStat, 6.542458, tolerance=1e-3)
  expect_equal(qrt23t$ch_res$indStat, -0.3604339, tolerance=1e-3)
  expect_equal(qrt23t$ch_res$stdError, 0.1397589, tolerance=1e-3)
