...
params23t <- c(1.8, 0.9, -0.06, 0.4, 7.2, 0.3, -0.009, 0.1, 3.1, 0.83, -0.05, 3.7, 0.7, 0.2, 11, 39, 18) # StMAR
...
test_that("quantile residual test for normality works", {
  ...
  expect_equal(qrt23t$norm_res$testStat, 8.278919, tolerance=1e-4)
  expect_equal(qrt23t$norm_res$pvalue, 0.04058564, tolerance=1e-4)
  expect_equal(qrt23t$norm_res$df, 3, tolerance=1e-4)
  expect_equal(qrt12r$norm_res$testStat, 1.574353, tolerance=1e-4)
  expect_equal(qrt12r$norm_res$pvalue, 0.6652191, tolerance=1e-4)
})

test_that("quantile residuals tests for autocorrelation work", {
  expect_equal(qrt12gs$ac_res$testStat, 1.227147, tolerance=1e-4)
  expect_equal(qrt12gs$ac_res$pvalue, 0.5414125, tolerance=1e-4)
  ...
  expect_equal(qrt12$ac_res$indStat, c(0.05724395, 0.03412973, 0.01891017, -0.00966325), tolerance=1e-4)
  expect_equal(qrt12$ac_res$pvalue, c(0.4052169, 0.5998024, 0.9192248, 0.8820040), tolerance=1e-4)
  expect_equal(qrt12$ac_res$stdError, c(0.06842647, 0.07189492, 0.05256620, 0.06739349), tolerance=1e-4)
  expect_equal(qrt23$ac_res$testStat, 1.18069, tolerance=1e-4)
  expect_equal(qrt23$ac_res$indStat, 0.1210913, tolerance=1e-4)
  expect_equal(qrt23$ac_res$stdError, 0.1108767, tolerance=1e-4)
  expect_equal(qrt23t$ac_res$testStat, 0.6537608, tolerance=1e-4)
  expect_equal(qrt23t$ac_res$pvalue, 0.8840221, tolerance=1e-4)
  expect_equal(qrt23t$ac_res$indStat, -0.02988711, tolerance=1e-4)
  expect_equal(qrt23t$ac_res$stdError, 0.08999044, tolerance=1e-4)
  expect_equal(qrt12r$ac_res$testStat, c(0.4761958, 1.4598831), tolerance=1e-4)
  expect_equal(qrt12r$ac_res$indStat, c(0.26191738, -0.06335654), tolerance=1e-4)
  expect_equal(qrt12r$ac_res$testStat, c(0.4761958, 1.4598831), tolerance=1e-4)
  expect_equal(qrt12$ac_res$stdError, c(0.06842647, 0.07189492, 0.05256620, 0.06739349), tolerance=1e-4)
})

test_that("quantile residual tests for conditional heteroskedasticity work", {
  expect_equal(qrt12gs$ch_res$testStat, 0.01772478, tolerance=1e-4)
  expect_equal(qrt12gs$ch_res$pvalue, 0.8940871, tolerance=1e-4)
  expect_equal(qrt13gsr$ch_res$indStat, 0.5920743, tolerance=1e-4)
  expect_equal(qrt11t$ch_res$testStat, c(0.3393064, 0.4802340, 2.5123071, 1.1701140), tolerance=1e-4)
  expect_equal(qrt11t$ch_res$stdError, c(0.7286874, 1.0237356, 0.8122795, 0.8237535), tolerance=1e-4)
  expect_equal(qrt12$ch_res$testStat, c(0.4647988, 0.5310061, 10.1436096, 20.4697598), tolerance=1e-4)
  expect_equal(qrt12$ch_res$pvalue, c(0.49538973, 0.76682012, 0.07126690, 0.02510988), tolerance=1e-3)
  expect_equal(qrt12$ch_res$indStat, c(-0.06665624, -0.01315376, -0.41151462, -0.37746047), tolerance=1e-4)
  expect_equal(qrt12$ch_res$stdError, c(0.09727555, 0.15172964, 0.13413233, 0.12248699), tolerance=1e-4)
  expect_equal(qrt23$ch_res$testStat, 18.6777, tolerance=1e-4)
  expect_equal(qrt23$ch_res$indStat, -0.3046883, tolerance=1e-4)
  expect_equal(qrt23$ch_res$stdError, 0.09125149, tolerance=1e-4)
  expect_equal(qrt23t$ch_res$testStat, 6.773131, tolerance=1e-4)
  expect_equal(qrt23t$ch_res$pvalue, 0.03382465, tolerance=1e-4)
  expect_equal(qrt23t$ch_res$indStat, -0.3624057, tolerance=1e-4)
  expect_equal(qrt23t$ch_res$stdError, 0.1382507, tolerance=1e-4)
  expect_equal(qrt12r$ch_res$testStat, c(0.07133745, 0.43758998), tolerance=1e-4)
  expect_equal(qrt12r$ch_res$testStat, c(0.07133745, 0.43758998), tolerance=1e-4)
  expect_equal(qrt12r$ch_res$indStat, c(0.1396549, -0.2607705), tolerance=1e-4)
  expect_equal(qrt12r$ch_res$stdError, c(0.5148911, 0.4147108), tolerance=1e-4)
})
