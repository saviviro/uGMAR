...
print.gsmar <- function(x, ..., digits=2, summary_print=FALSE) {
  ...
  for(m in seq_len(sum(M))) {
    ...
    for(i1 in seq_len(p)) {
      cat(paste0(" + [", format_value(pars[1 + i1, m]), "]Y.l", i1))
...
#'
#' @description \code{print.gsmarsum} is a print method for object \code{'gsmarsum'} generated
#'   by \code{summary.gsmar()}. Approximate tandard errors are printed in brackets.
#'
...
#'
#' @description \code{print.gsmarpred} is print method for object generated
...
print.gsmarpred <- function(x, ..., digits=2) {
  ...
  if(gsmarpred$pred_type == "cond_mean") {
  ...
  } else if(gsmarpred$ci_type == "none") {
    cat(paste0("Prediction by ", gsmarpred$pred_type, ", no confidence intervals"), "\n")
    ...
    cat(paste0("Prediction by ", gsmarpred$pred_type, ", ", gsmarpred$ci_type,
               " confidence intervals with levels ", paste(gsmarpred$ci, collapse=", "), ""), "\n")
    cat(paste0("Forecast ", gsmarpred$n_ahead, " steps ahead, based on ", gsmarpred$nsimu, " simulations\n"))

    cat("\n")
    q <- gsmarpred$q
    conf_ints <- gsmarpred$conf_ints
    pred_type <- gsmarpred$pred_type
    df <- as.data.frame(lapply(1:ncol(conf_ints), function(i1) format_value(conf_ints[,i1])))
    names(df) <- q
...
#' @describeIn quantileResidualTests print method for class 'qrtest'
#' @param x object of class \code{'qrtest'} generated by the function \code{quantileResidualTests()}.
...
print.qrtest <- function(x, ..., digits=3) {
  ...
  for(i1 in seq_along(qrtest$ac_res$lags)) {
    if(qrtest$ac_res$lags[i1] < 10) {
      cat(" ", format_lag(qrtest$ac_res$lags[i1]), " | ", format_value(qrtest$ac_res$pvalue[i1]), "\n")
    } else {
      cat(" ", format_lag(qrtest$ac_res$lags[i1]), "| ", format_value(qrtest$ac_res$pvalue[i1]), "\n")
    }
  }
  cat("\nConditional hetetoskedasticity tests:\nlags | p-value\n")
  for(i1 in seq_along(qrtest$ch_res$lags)) {
    if(qrtest$ch_res$lags[i1] < 10) {
      cat(" ", format_lag(qrtest$ch_res$lags[i1]), " | ", format_value(qrtest$ch_res$pvalue[i1]), "\n")
    } else {
      cat(" ", format_lag(qrtest$ch_res$lags[i1]), "| ", format_value(qrtest$ch_res$pvalue[i1]), "\n")
    }
  }
